...
export default defineEventHandler(async (event) => {
  ...
  const { motif, palette, title } = await getValidatedQuery(event, previewSchema.parse)

  setResponseHeader(event, 'content-type', 'image/svg+xml; charset=utf-8')
  return renderCover({ id: `preview-${motif}-${palette}`, title, motif, palette, variant: 'card' })
})
