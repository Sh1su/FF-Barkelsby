...
describe('FV-12 Konto-CLI – Kennung und Passwort ohne Anmeldung', () => {
  ...
  it('AC-1: listet alle Konten mit Kennung, Rolle, Zustand und Wechselhinweis', async () => {
    ...
    expect(ausgabe).toContain(ACCOUNTS.memberEmail)
    expect(ausgabe).toContain('admin')
    expect(ausgabe).toContain('member')
  ...
  it('AC-2: ändert die Kennung und normalisiert Leerzeichen und Großschreibung', async () => {
    ...
    const code = await runAccountCommand(
      db,
      ['set-email', 'member', '  MITGLIED-NEU@Example.ORG '],
      io,
      dbPath,
    )

    expect(code).toBe(0)
    expect(konto('mitglied-neu@example.org')?.role).toBe('member')
    expect(konto(ACCOUNTS.memberEmail)).toBeUndefined()
  })
  ...
  it('AC-3: lehnt eine bereits vergebene Kennung ab und lässt beide Konten unverändert', async () => {
    ...
    const code = await runAccountCommand(
      db,
      ['set-email', 'admin', ACCOUNTS.memberEmail],
      io,
      dbPath,
    )
    ...
    expect(konto(ACCOUNTS.memberEmail)?.role).toBe('member')
  })

  it('AC-4: fragt das Passwort zweimal verdeckt ab', async () => {
    const { io, prompts } = fakeIO(['ein-langes-passwort', 'ein-langes-passwort'])
    const code = await runAccountCommand(db, ['set-password', 'member'], io, dbPath)
  ...
  it('AC-4: bricht ab, wenn die Bestätigung abweicht', async () => {
    ...
    expect(
      await verifyPasswordHash(konto(ACCOUNTS.memberEmail)!.passwordHash, ACCOUNTS.memberPassword),
    ).toBe(true)
  })
  ...
  it('AC-4: liest mit --passwort-stdin genau einmal, ohne Rückfrage', async () => {
    ...
    const code = await runAccountCommand(
      db,
      ['set-password', 'member', '--passwort-stdin'],
      io,
      dbPath,
    )
    ...
    expect(
      await verifyPasswordHash(konto(ACCOUNTS.memberEmail)!.passwordHash, 'aus-der-standardeingabe'),
    ).toBe(true)
  })
  ...
  it('AC-4: nimmt ein Passwort als Argument nicht entgegen', async () => {
    ...
    const code = await runAccountCommand(
      db,
      ['set-password', 'member', 'geheim-und-lang-genug'],
      io,
      dbPath,
    )
  ...
  it('AC-6: setzt das Passwort sofort gültig und ohne erzwungenen Wechsel', async () => {
    ...
    expect(code).toBe(0)
    const mitglied = konto(ACCOUNTS.memberEmail)!
    expect(await verifyPasswordHash(mitglied.passwordHash, 'frisches-passwort-123')).toBe(true)
    expect(await verifyPasswordHash(mitglied.passwordHash, ACCOUNTS.memberPassword)).toBe(false)
    expect(mitglied.mustChangePassword).toBe(false)
  })
  ...
  it('AC-6: erzwingt den Wechsel mit --wechsel-erzwingen', async () => {
    ...
    const code = await runAccountCommand(
      db,
      ['set-password', 'member', '--wechsel-erzwingen'],
      io,
      dbPath,
    )

    expect(code).toBe(0)
    expect(konto(ACCOUNTS.memberEmail)?.mustChangePassword).toBe(true)
  })

  it('AC-7: trifft über die Kurzformen member und admin genau ein Konto', async () => {
    const { io } = fakeIO()
    expect(await runAccountCommand(db, ['set-email', 'member', 'g@test.local'], io, dbPath)).toBe(0)
    expect(await runAccountCommand(db, ['set-email', 'admin', 'a@test.local'], io, dbPath)).toBe(0)

    expect(konto('g@test.local')?.role).toBe('member')
  ...
  it('AC-8: weist einen unbekannten Befehl mit Hilfetext zurück', async () => {
    const { io, err } = fakeIO()
    const code = await runAccountCommand(db, ['set-mail', 'member', 'x@test.local'], io, dbPath)
  ...
  it('AC-8: weist fehlende Angaben und unbekannte Optionen zurück', () => {
    ...
    expect(parseCommand(['set-email', 'member'])).toMatchObject({ kind: 'usage-error' })
    expect(parseCommand(['list', 'zuviel'])).toMatchObject({ kind: 'usage-error' })
    expect(parseCommand(['set-password', 'member', '--was-auch-immer'])).toMatchObject({
      kind: 'usage-error',
    })
  ...
  it('AC-9: gibt weder Passwort noch Hash aus – auch nicht im Fehlerfall', async () => {
    ...
    await runAccountCommand(db, ['set-password', 'member'], erfolg.io, dbPath)

    const fehler = fakeIO(['zu-kurz', 'zu-kurz'])
    await runAccountCommand(db, ['set-password', 'member'], fehler.io, dbPath)
